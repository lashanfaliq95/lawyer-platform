    ...
    <ListGroup className="user-details-list">
      <ListGroupItem className="list-item">
        <div className="initials-icon">
          {userAppointmentDetails.initials}
        </div>
      ...
      <ListGroupItem>
        <span className="list-item-title">{formatMessages(messages.legalIssue)}</span>
        <br />
        {userAppointmentDetails.legalIssue}
      </ListGroupItem>
      <ListGroupItem>
        <span className="list-item-title">{formatMessages(messages.typeOfAppointment)}</span>
        <br />
        {userAppointmentDetails.typeOfAppointment}
      </ListGroupItem>
      <ListGroupItem className="address">
        <div>
          <span className="list-item-title">{formatMessages(messages.address)}</span>
          <br />
          {userAppointmentDetails.address}
        </div>
        <div className="directions">
          <a href={`https://www.google.com/maps/search/?api=1&query=${userAppointmentDetails.lat},${userAppointmentDetails.lon}`} target="_blank" rel="noreferrer">
            {formatMessages(messages.directions)}
          </a>
        </div>
      </ListGroupItem>
      <ListGroupItem>
        <span className="list-item-title">{formatMessages(messages.access)}</span>
        <br />
        {userAppointmentDetails.access}
      </ListGroupItem>
      <ListGroupItem>
        <span className="list-item-title">{formatMessages(messages.phoneNumber)}</span>
        <br />
        {userAppointmentDetails.phoneNumber}
      </ListGroupItem>
      <ListGroupItem>
        <span className="list-item-title">{formatMessages(messages.standardMessage)}</span>
        <br />
        {userAppointmentDetails.standardMessage}
      </ListGroupItem>
      {!isPastAppointments ? (
        <ListGroupItem className="list-footer">
          <Button color="secondary" onClick={() => toggleCancellationModal(true)}>
